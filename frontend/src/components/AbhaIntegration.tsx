  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    api.post('/abha/', formData)
      .then(() => {
        ...
        setFormData({ abha_number: '', abha_address: '', aadhaar_last4: '', patient_name: '', gender: 'Male', dob: '', mobile: '' });
        fetchRecords();
      })
      .catch((err) => console.error('Error creating ABHA record:', err));
  };
      ...
      <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm md:col-span-1">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input type="text" required placeholder="91-XXXX-XXXX-XXXX" className="w-full p-2 border rounded text-sm mt-1"
              value={formData.abha_number} onChange={(e) => setFormData({...formData, abha_number: e.target.value})} />
          </div>
          <div>
            <label className="text-xs font-semibold text-gray-600">ABHA Address / Username</label>
            <input type="text" required placeholder="name@abdm" className="w-full p-2 border rounded text-sm mt-1"
              value={formData.abha_address} onChange={(e) => setFormData({...formData, abha_address: e.target.value})} />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs font-semibold text-gray-600">Aadhaar (Last 4)</label>
              <input type="text" maxLength={4} required className="w-full p-2 border rounded text-sm mt-1"
                value={formData.aadhaar_last4} onChange={(e) => setFormData({...formData, aadhaar_last4: e.target.value})} />
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {records.map((rec) => (
            <div key={rec.id} className="p-4 border border-teal-200 bg-teal-50/50 rounded-lg relative">
              ...
              <p className="text-xs text-gray-400 mt-2">Mobile: {rec.mobile} | Aadhaar: XXXX-XXXX-{rec.aadhaar_last4}</p>
            </div>
          ))}
        </div>
      </div>
